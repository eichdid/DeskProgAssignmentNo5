...
public class MainViewController {
    ...
    private void showDatabaseError(){
        JOptionPane.showMessageDialog(null, "Gagal mengakses database.");
    }
    ...
    public void eraseCurrentProfile(){
        ...
        if (JOptionPane.showConfirmDialog(null, 
                "Apakah anda yakin ingin menghapus profil '" + currentProfile.getAlias() + "' ?"
                , "Diterima", JOptionPane.YES_NO_OPTION) == 1) return;
        try {
            storage.remove(currentProfile);
            currentProfile = null;
            JOptionPane.showMessageDialog(null, "Profil dihapus.");
        } catch (DatabaseException ex) {
            Logger.getLogger(MainViewController.class.getName()).log(Level.SEVERE, null, ex);
            showDatabaseError();
        }
